...
export default async function DashboardPage() {
    ...
    const tier = stats ? getReliabilityTier(stats.reliabilityScore ?? 0) : getReliabilityTier(100);
    ...
    const overdueCount = activeTasks.filter(
        (t) => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== "SUBMITTED"
    ).length;
    ...
    // Builder profile summary (from new onboarding model)
    const ob = user.onboarding;

    return (
        <div className="w-full  mx-auto p-5 md:p-8 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <QuickStat
                    ...
                    unit="%"
                    color="#60a5fa"
                    sub={totalTerminal > 0 ? `${totalTerminal} tasks total` : "No data yet"}
                />
                <QuickStat
                    label="Active tasks"
                    value={String(activeTasks.length)}
                    unit=""
                    color={activeTasks.length > 4 ? "#fb923c" : "var(--text)"}
                    sub={`${activeTasks.filter((t) => t.status === "SUBMITTED").length} awaiting review`}
                />
                <QuickStat
                    label="Projects"
                    value={String(user.teams.length)}
                    unit=""
                    color="var(--text)"
            ...
            <div className="flex flex-col md:flex-row gap-5 items-start">
                ...
                <div className="flex flex-col gap-4 flex-1 min-w-0 w-full">
                    ...
                    {activeTasks.length > 0 ? (
                        <div className="p-5 rounded-2xl border border-[var(--border)] bg-[var(--surface)]">
                            <p className="text-xs font-medium text-[var(--text)] uppercase tracking-wider mb-3">
                                Your tasks
                            </p>
                            <div className="flex flex-col gap-1.5">
                                {activeTasks.map((task) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span
                                                className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${task.status === "SUBMITTED"
                                                        ? "bg-amber-400"
                                            ...
                                            {task.dueDate ? (
                                                <span
                                                    className={`text-[10px] flex-shrink-0 font-medium ${overdue
                                                            ? "text-red-400"
                                                            : dueToday
                                                                ? "text-amber-400"
                                                                : "text-[var(--muted)]"
                    ...
                    {user.teams.length > 0 && (
                        <div className="p-5 rounded-2xl border border-[var(--border)] bg-[var(--surface)]">
                            <div className="flex items-center justify-between mb-3">
                                <p className="text-xs font-medium text-[var(--text)] uppercase tracking-wider">
                                    Your projects
                                </p>
                                <Link
                                    href="/projects"
                                    className="text-[10px] text-[var(--muted)] hover:text-[var(--text)] no-underline"
                                >
                                    All projects →
                                </Link>
                            </div>
                            <div className="flex flex-col gap-2">
                                {user.teams.map((t) => (
                                    ...
                                    >
                                        ...
                                        <span
                                            className="text-[9px] font-semibold flex-shrink-0 uppercase"
                                            style={{
                                                color: PROJECT_STATUS_COLORS[t.project.status] ?? "#666",
                                            }}
                                        >
                                            {t.project.status}
                                        </span>
                ...
                {/* RIGHT — builder profile + notifications */}
                <div className="flex flex-col gap-4 w-full md:w-56 md:flex-shrink-0">
                    ...
                    {ob ? (
                        <div className="p-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)]">
                            <div className="flex items-center justify-between mb-3">
                                <p className="text-xs font-medium text-[var(--text)]">Builder</p>
                                <Link
                                    href="/profile"
                                    className="text-[10px] text-[var(--muted)] hover:text-[var(--text)] no-underline"
                                >
                            ...
                            <p className="text-sm font-semibold text-[var(--text)] mb-0.5 truncate">
                                {ob.role.replace(/_/g, " ")}
                            </p>
                            <p className="text-[10px] text-[var(--muted)] mb-3">
                                {ob.experienceLevel.charAt(0) + ob.experienceLevel.slice(1).toLowerCase()} ·{" "}
                                {ob.domain.replace(/_/g, " ")}
                            </p>

                            {/* Builder score bar */}
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-[10px] text-[var(--muted)]">Builder score</span>
                            ...
                            <div className="h-1 bg-[var(--surface2)] rounded-full overflow-hidden">
                                <div
                                    ...
                                    style={{ width: `${ob.builderScore}%` }}
                                />
                            </div>
                        </div>
                    ) : (
                        <div className="p-4 rounded-2xl border border-dashed border-[var(--border)] text-center">
                            <p className="text-xs text-[var(--muted)] mb-2">Profile incomplete</p>
                            <Link
                                href="/onboarding"
                                ...
                                {recentNotifications.map((n) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[10px] text-[var(--muted)] line-clamp-1 mt-0.5">
                                                {n.body}
                                            </p>
                                        </div>
                                    </Link>
    ...
    const inner = (
        <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] flex flex-col gap-1 hover:border-[var(--muted)] transition-colors h-full">
            ...
            <p className="text-xl font-bold" style={{ color }}>
                {value}
                <span className="text-xs font-normal ml-0.5 text-[var(--muted)]">{unit}</span>
            </p>
